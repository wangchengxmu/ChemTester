...
Both native skills and the corpus-backed router are recommended options.
Native skills are lighter and more efficient, and are the recommended
installation for most users. The router can improve answering performance
through selective access to the broader knowledge library, but adds orchestration
latency. Gains depend on the model and questions rather than being guaranteed.

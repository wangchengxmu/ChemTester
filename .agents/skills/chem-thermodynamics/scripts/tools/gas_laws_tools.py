...
def _require_positive(name: str, value: Optional[float]) -> Optional[float]:
    if value is None:
        return None
    numeric = _real_value(name, value)
    if numeric <= 0:
...
def _require_one_missing(args: list[Optional[float]]) -> None:
    if sum(value is None for value in args) != 1:
        raise ValueError("Exactly three of four values must be provided")
...
def boyles_law(P1: Optional[float] = None, V1: Optional[float] = None,
    ...
    """
    args = [P1, V1, P2, V2]
    _require_one_missing(args)
    P1, V1, P2, V2 = (
        _require_positive(name, value)
        for name, value in zip(("P1", "V1", "P2", "V2"), args)
    )
...
def charles_law(V1: Optional[float] = None, T1: Optional[float] = None,
    ...
    V1, T1, V2, T2 = (
        _require_positive(name, value)
        for name, value in zip(("V1", "T1", "V2", "T2"), args)
    )
...
def gay_lussacs_law(P1: Optional[float] = None, T1: Optional[float] = None,
    ...
    P1, T1, P2, T2 = (
        _require_positive(name, value)
        for name, value in zip(("P1", "T1", "P2", "T2"), args)
    )
...
def avogadros_law(V1: Optional[float] = None, n1: Optional[float] = None,
    ...
    V1, n1, V2, n2 = (
        _require_positive(name, value)
        for name, value in zip(("V1", "n1", "V2", "n2"), args)
    )
...
def combined_gas_law(P1: Optional[float] = None, V1: Optional[float] = None, 
    ...
    If n (moles) is provided, the complete initial or final state is checked
    against nR within a relative tolerance of 1e-3 for rounded inputs. The
    standard combined-gas-law equation is then used to solve the missing
    state value.
    
    Args:
        P1, V1, T1: Initial pressure, volume, and temperature (K)
        P2, V2, T2: Final pressure, volume, and temperature (K)
        n: Optional moles of gas used as a consistency check
    ...
    Examples:
        ...
        2.0
        >>> # With n provided, the complete state is checked against nR
        >>> combined_gas_law(P1=1.0, V1=0.08206 * 300, T1=300, P2=2.0, V2=0.08206 * 125, n=1.0)
        250.0
    """
    args = [P1, V1, T1, P2, V2, T2]
    if sum(a is None for a in args) != 1:
        raise ValueError("Exactly five of six state values must be provided")
    P1, V1, T1, P2, V2, T2 = (
        _require_positive(name, value)
        for name, value in zip(("P1", "V1", "T1", "P2", "V2", "T2"), args)
    )
    n = _require_positive("n", n)
    R = _require_positive("R", R)

    if n is not None:
        if P1 is not None and V1 is not None and T1 is not None:
            complete_state_ratio = P1 * V1 / T1
        else:
            complete_state_ratio = P2 * V2 / T2
        if not isclose(complete_state_ratio, n * R, rel_tol=1e-3):
            raise ValueError(
                "Complete gas state is inconsistent with n*R "
                "within relative tolerance 1e-3"
            )
...
def celsius_to_kelvin(celsius: float) -> float:
    ...
    celsius = _real_value("celsius", celsius)
    kelvin = celsius + 273.15
    _require_positive("kelvin", kelvin)
    return kelvin


def kelvin_to_celsius(kelvin: float) -> float:
    """Convert Kelvin to Celsius."""
    kelvin = _require_positive("kelvin", kelvin)
...
def partial_pressure_dalton(mole_fraction: float, total_pressure: float) -> float:
    ...
    Examples:
        ...
        1.0
    """
    mole_fraction = _require_nonnegative("mole_fraction", mole_fraction)
    total_pressure = _require_positive("total_pressure", total_pressure)
    if mole_fraction > 1:
        raise ValueError("mole_fraction must be at most 1")
...
def mole_fraction(moles_component: float, total_moles: float) -> float:
    ...
    Examples:
        ...
        0.5
    """
    moles_component = _require_nonnegative("moles_component", moles_component)
    total_moles = _require_positive("total_moles", total_moles)
    if moles_component > total_moles:
        raise ValueError("moles_component cannot exceed total_moles")
...
def dalton_law_partial_pressures(moles_dict: dict, total_pressure: float) -> dict:
    ...
    """
    total_pressure = _require_positive("total_pressure", total_pressure)
    if not isinstance(moles_dict, dict) or not moles_dict:
        raise ValueError("moles_dict must be a non-empty dictionary")
    moles_dict = {
        species: _require_nonnegative(f"moles_dict[{species!r}]", moles)
        for species, moles in moles_dict.items()
    }
    total_moles = sum(moles_dict.values())
    _require_positive("total_moles", total_moles)
    partial_pressures = {}
    for species, moles in moles_dict.items():
...
MCP_TOOLS = [
    ...
    },
    {
        ...
        "description": "Apply Combined Gas Law: P₁V₁/T₁ = P₂V₂/T₂ (constant n). If n is provided, check the complete state against nR before solving.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "P1": {"type": "number", "description": "P1", "default": None},
                ...
                "V2": {"type": "number", "description": "V2", "default": None},
                "T2": {"type": "number", "description": "T2", "default": None},
                "n": {"type": "number", "description": "Moles of gas (optional consistency check)", "default": None},
